...
void UK2Node_SpawnAndBindActor::AllocateDefaultPins()
{
	...
	UEdGraphPin* const CollisionHandlingOverridePin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Byte, MethodEnum, FK2Node_SpawnAndBindActorHelper::CollisionHandlingOverridePinName);
...
void UK2Node_SpawnAndBindActor::GetPinHoverText(const UEdGraphPin& Pin, FString& HoverTextOut) const
{
...
void UK2Node_SpawnAndBindActor::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	// 创建 InitializeActorLifetime 并在执行 end 之前调用
	static const FName InitActorLifetimeStateFuncName = GET_FUNCTION_NAME_CHECKED(UFSMFunctionLibrary, InitActorLifetimeState);
	...
	UEdGraphPin* LifetimePin = SpawnNode->GetLifetimePin();
	UEdGraphPin* ActorTagPin = SpawnNode->GetActorTagPin();
	...
	//////////////////////////////////////////////////////////////////////////
	// create 'initialize lifetime' call node
	UK2Node_CallFunction* CallInitLifetimeNode = CompilerContext.SpawnIntermediateNode<UK2Node_CallFunction>(SpawnNode, SourceGraph);
	CallInitLifetimeNode->FunctionReference.SetExternalMember(InitActorLifetimeStateFuncName, UFSMFunctionLibrary::StaticClass());
	CallInitLifetimeNode->AllocateDefaultPins();

	UEdGraphPin* CallInitLifetimeExec = CallInitLifetimeNode->GetExecPin();
	UEdGraphPin* CallInitLifetimeThen = CallInitLifetimeNode->GetThenPin();
	UEdGraphPin* CallInitTarget = CallInitLifetimeNode->FindPinChecked(TargetParamName);
	UEdGraphPin* CallInitLifetime = CallInitLifetimeNode->FindPinChecked(LifetimeParamName);
	UEdGraphPin* CallInitActorTag = CallInitLifetimeNode->FindPinChecked(ActorTagParamName);

	// Connect output actor from 'begin' to 'finish'
	CallBeginResult->MakeLinkTo(CallInitTarget);

	if (LifetimePin != nullptr)
	{
		CompilerContext.MovePinLinksToIntermediate(*LifetimePin, *CallInitLifetime);
	}
	if (ActorTagPin != nullptr)
	{
		CompilerContext.MovePinLinksToIntermediate(*ActorTagPin, *CallInitActorTag);
	}
	...
	// 将属性初始化后的连线接入初始化生命周期函数
	LastThen->MakeLinkTo(CallInitLifetimeExec);
	
	// 初始化生命周期完成后调用结束生成函数
	CallInitLifetimeThen->MakeLinkTo(CallFinishExec);
...
void UK2Node_SpawnAndBindActor::GetNodeAttributes(TArray<TKeyValuePair<FString, FString>>& OutNodeAttributes) const
{
	...
	OutNodeAttributes.Add( TKeyValuePair<FString, FString>( TEXT( "Lifetime" ), TEXT("Actor Lifetime") ));
	OutNodeAttributes.Add( TKeyValuePair<FString, FString>( TEXT( "ActorTag" ), TEXT("Add A Actor Tag") ));
}
...
bool UK2Node_SpawnAndBindActor::IsSpawnVarPin(UEdGraphPin* Pin) const
{
	...
	return(	Super::IsSpawnVarPin(Pin) &&
			...
			Pin->PinName != FK2Node_SpawnAndBindActorHelper::LifetimeName &&
			Pin->PinName != FK2Node_SpawnAndBindActorHelper::ActorTagName);
}

FText UK2Node_SpawnAndBindActor::GetBaseNodeTitle() const
{
	return LOCTEXT("SpawnActor_BaseTitle", "Spawn and Bind Actor from Class");
}

FText UK2Node_SpawnAndBindActor::GetDefaultNodeTitle() const
{
	return LOCTEXT("SpawnActor_Title_NONE", "SpawnAndBindActor NONE");
}

FText UK2Node_SpawnAndBindActor::GetNodeTitleFormat() const
{
	return LOCTEXT("SpawnActor_Title_Class", "SpawnAndBindActor {ClassName}");
}
...
{
	UEdGraphPin* Pin = FindPin(FK2Node_SpawnAndBindActorHelper::OwnerPinName);
