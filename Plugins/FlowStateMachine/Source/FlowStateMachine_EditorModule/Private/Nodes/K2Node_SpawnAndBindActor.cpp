...
void UK2Node_SpawnAndBindActor::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	static const  FName InitObjectFuncName = GET_FUNCTION_NAME_CHECKED(UFlowStateBFL, InitActorLifetimeState);
	...
	FName FindActorFuncName = (GET_FUNCTION_NAME_CHECKED(UFlowStateBFL, FindActorFromCache));
	...
	CallFindActorNode->FunctionReference.SetExternalMember(FindActorFuncName, UFlowStateBFL::StaticClass());
	...
	CallInitObjectNode->FunctionReference.SetExternalMember(InitObjectFuncName, UFlowStateBFL::StaticClass());
	...
	for(int32 PinIdx=0; PinIdx < Pins.Num(); PinIdx++)
	{
		...
		if (SpawnVarPin->PinName == FK2Node_SpawnAndBindActorHelper::MeshName)
		{
			...
			{
				FName FuncName = GET_FUNCTION_NAME_CHECKED(UFlowStateBFL, SetActorStaticMesh);
				MeshNode_OnSpawn = CompilerContext.SpawnIntermediateNode<UK2Node_CallFunction>(this, SourceGraph);
				MeshNode_OnSpawn->FunctionReference.SetExternalMember(FuncName, UFlowStateBFL::StaticClass());

				MeshNode_OnFind = CompilerContext.SpawnIntermediateNode<UK2Node_CallFunction>(this, SourceGraph);
				MeshNode_OnFind->FunctionReference.SetExternalMember(FuncName, UFlowStateBFL::StaticClass());
			}
			else if (ClassToSpawn == ASkeletalMeshActor::StaticClass())
			{
				FName FuncName = GET_FUNCTION_NAME_CHECKED(UFlowStateBFL, SetActorSkeletalMesh);
				MeshNode_OnSpawn = CompilerContext.SpawnIntermediateNode<UK2Node_CallFunction>(this, SourceGraph);
				MeshNode_OnSpawn->FunctionReference.SetExternalMember(FuncName, UFlowStateBFL::StaticClass());
