...
const FName FFSMGraphEditor::FlowStateMachineMode = FName("FlowStateMachine");
const FName FFSMGraphEditor::CommonDataMode       = FName("CommonData");
    ...
    const TSharedPtr<class IToolkitHost>& InitToolkitHost, UObject* InObject)
{
    ...
    if (EditedObjects == nullptr || EditedObjects->Num() == 0)
    {
        ...
        InitAssetEditor(
            Mode,
            InitToolkitHost,
            FFlowStateEditorModule::FlowStateMachineAppIdentifier,
...
{
    const UObject* EditingObject = GetCurrentMode() == FlowStateMachineMode ? static_cast<UObject*>(FlowStateMachine) : static_cast<UObject*>(CommonData);
