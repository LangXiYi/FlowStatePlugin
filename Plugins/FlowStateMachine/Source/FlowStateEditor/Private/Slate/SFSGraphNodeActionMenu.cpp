...
SFSGraphNodeActionMenu::~SFSGraphNodeActionMenu()
{
    OnClosedCallback.ExecuteIfBound();
}

void SFSGraphNodeActionMenu::Construct(const FArguments& InArgs)
{
    ...
    SBorder::Construct(SBorder::FArguments()
        ...
        [
            ...
            [
                ...
                .OnActionSelected(this, &SFSGraphNodeActionMenu::OnActionSelected)
                .OnCollectAllActions(this, &SFSGraphNodeActionMenu::CollectAllActions)
...
void SFSGraphNodeActionMenu::CollectAllActions(FGraphActionListBuilderBase& OutAllActions)
{
    ...
    const UFlowStateGraphSchema* MySchema = Cast<const UFlowStateGraphSchema>(GraphObj->GetSchema());
    if (MySchema)
    {
        // 调用获取节点行为函数
...
TSharedRef<SEditableTextBox> SFSGraphNodeActionMenu::GetFilterTextBox()
{
    return GraphActionMenu->GetFilterTextBox();
}

void SFSGraphNodeActionMenu::OnActionSelected(const TArray<TSharedPtr<FEdGraphSchemaAction>>& SelectedAction,
                                              ESelectInfo::Type InSelectionType)
{
    if (InSelectionType == ESelectInfo::OnMouseClick || InSelectionType == ESelectInfo::OnKeyPress || SelectedAction.
        Num() == 0)
    {
