...
TSharedRef<SGraphNode> SFSGraph_StateNode::GetNodeUnderMouse(const FGeometry& MyGeometry,
...
TSharedRef<SWidget> SFSGraph_StateNode::CreateTitleWidget(TSharedPtr<SNodeTitle> NodeTitle)
{
    ...
    InlineEditableText->SetColorAndOpacity(
        TAttribute<FLinearColor>::Create(
            TAttribute<FLinearColor>::FGetter::CreateSP(this, &SGraphNode::GetNodeTitleTextColor)));
...
{
    return Pin
               ? /*Pin->bAllowFriendlyName ? Pin->PinFriendlyName :*/ FText::FromName(Pin->PinName)
