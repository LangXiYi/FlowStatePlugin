...
public:
    ...
    /** 两节点间连接的条件 */
    virtual const FPinConnectionResponse
    CanCreateConnection(const UEdGraphPin* PinA, const UEdGraphPin* PinB) const override;
    /** 两节点是否可以合并（未完成） */
    ...
    virtual void GetGraphNodeContextActions(FGraphContextMenuBuilder& ContextMenuBuilder,
                                            FSET::ESubNodeType SubNodeFlags) const;
    virtual void GetSubNodeClasses(FSET::ESubNodeType SubNodeFlags, TArray<FGraphNodeClassData>& ClassData,
                                   UClass*& GraphNodeClass) const;

    // 供 GraphPalette 使用，获取所有可添加的列表
    static void CollectNewNodeAction(FCategorizedGraphActionListBuilder& TasksBuilder, UClass* NodeInstanceClass,
                                     UClass* GraphNodeClass, const UEdGraph* InGraph);
    static void CollectNewSubNodeAction(FCategorizedGraphActionListBuilder& TasksBuilder, UClass* NodeInstanceClass,
                                        UClass* GraphNodeClass, const UEdGraph* InGraph);
    static void CollectJumpNodeAction(FCategorizedGraphActionListBuilder& TasksBuilder,
...
protected:
    static TSharedPtr<FFSMSchemaAction_NewNode> AddNewNodeAction(FGraphActionListBuilderBase& ContextMenuBuilder,
                                                                 const FText& Category, const FText& MenuDesc,
                                                                 const FText& Tooltip);
    static TSharedPtr<FFSMSchemaAction_NewJumpNode> AddNewJumpNodeAction(
        FGraphActionListBuilderBase& ContextMenuBuilder, const FText& Category, const FText& MenuDesc,
        const FText& Tooltip);
    static TSharedPtr<FFSMSchemaAction_NewSubNode> AddNewSubNodeAction(FGraphActionListBuilderBase& ContextMenuBuilder,
                                                                       const FText& Category, const FText& MenuDesc,
                                                                       const FText& Tooltip);
};
