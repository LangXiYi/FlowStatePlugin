...
void FSMGC::KillCache(ECacheType Type)
{
    switch (Type)
    {
    case ECacheType::None:
...
void FSMGC::KillActorCache()
{
    ...
    for (TWeakObjectPtr<AActor> KillActor : Hidden_Actors)
    {
        if (KillActor.IsValid())
        {
            _KillCacheFrom(KillActor.Get());
    ...
    for (TWeakObjectPtr<AActor> KillActor : Static_Actors)
    {
        if (KillActor.IsValid())
        {
            _KillCacheFrom(KillActor.Get());
...
void FSMGC::KillWidgetCache()
{
    ...
    Kill_Widgets.Empty();
    for (TWeakObjectPtr<UWidget> KillWidget : Hidden_Widgets)
    {
        if (KillWidget.IsValid())
        {
            _KillCacheFrom(KillWidget.Get());
    ...
    for (TWeakObjectPtr<UWidget> KillWidget : Static_Widgets)
    {
        if (KillWidget.IsValid())
        {
            _KillCacheFrom(KillWidget.Get());
...
void FSMGC::KillLevelCache()
{
    ...
    for (FName KillSubLevel : Hidden_SubLevels)
    {
        if (KillSubLevel != NAME_None)
        {
            _KillCacheFrom(KillSubLevel);
    ...
    for (FName KillSubLevel : Static_SubLevels)
    {
        if (KillSubLevel != NAME_None)
        {
            _KillCacheFrom(KillSubLevel);
