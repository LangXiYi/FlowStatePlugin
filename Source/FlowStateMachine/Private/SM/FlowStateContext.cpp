...
bool UFlowStateContext::GotoScatteredNode(const FString& NodeName)
{
    for (auto Item : ScatteredNodeMapping) 
    {
        if (Item.Value->GetNodeName().Equals(NodeName)) 
        {
            return GotoScatteredNode(Item.Key);
        }
    }
    return false;
}
...
void UFlowStateContext::Tick(float DeltaTime)
{
    ...
    if (GCManager)
    {
        ...
        if (GEngine)
        {
            ...
            FString FormatStr = "{CacheInfo} \n State = {StateName}";
            Args.Add("CacheInfo", CacheInfo.ToString());
            Args.Add("StateName", CurState->GetNodeName());
            FString DebugStr = FString::Format(*FormatStr, Args);
            GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Green, DebugStr);
        }
    }
...
void UFlowStateContext::KillActorCache()
{
    GCManager->KillActorCache();
}

void UFlowStateContext::KillAllCache()
{
    GCManager->KillAllCache();
}
