...
void UActions_LoadAsset::LoadAsset(bool IsSyncLoad, bool IsPreload)
{
	...
	else
	{
		auto LoadCallbackFunc = [this]()
		{
                ...
                // 完成资产加载
                UFlowStateBase* State = UFSMFunctionLibrary::GetCurFlowState(this);
                if (State == ParentNode) 
                {
                    UE_LOG(LogTemp, Log, TEXT("Load Asset Completed, Try to switch other pin."))
                    State->SwitchToByName("LoadAssetCompleted");
                }
		};
...
void UActions_LoadAsset::GetStatePinInfos(TArray<FStatePinInfo>& PinInfos) const
{
    PinInfos.Add(FStatePinInfo("ThenPins", "LoadAssetCompleted"));
